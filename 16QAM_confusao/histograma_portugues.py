...
directory = r'D:\Rec\Mestrado\Codigo Final\16QAM_confusao\data_hist'
Nr = 256
Nw = 100
...
fig.add_annotation(
    x=0.58,   # caixa deslocada para a direita
...
fig.update_layout(
    title=dict(
        text=f"Histograma de Confusão - {scenario_name}",
        x=0.5,
        font=dict(size=28)
    ),
    ...
    xaxis=dict(
        title=dict(
            text="Distância de Hamming Normalizada d/Nr",
            font=dict(size=24)
        ),
